...
def get_total_for_row(row: list)->int:
    """
...
def main(quantity):
    ...
    result = []
    for row in values:
        total = get_total_for_row(row)
        new_row = [row[0], str(total), row[2], row[3]+" ,%i плат %d.%d.%d" % (quantity, now.day, now.month, now.year), row[4]]
    ...
    for i in lssound_indexes:
        row = values[i]
        total = get_total_for_row(row)
        new_row = [row[0], str(total), row[2], row[3] + " ,%i плат %d.%d.%d" % (quantity, now.day, now.month, now.year),
                   row[4]]
